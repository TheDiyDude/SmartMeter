...
# SDM72D: Positive refers to forward current, negative refers to reverse current.
L1_PF = 0x158, 0x0036, 0x001E
# SDM72D: Positive refers to forward current, negative refers to reverse current.
L2_PF = 0x158, 0x0038, 0x0020
# SDM72D: Positive refers to forward current, negative refers to reverse current.
L3_PF = 0x158, 0x003A, 0x0022
# SDM72D: Total active energy equals to import + export
Total_ActiveEnergy = -1, 0x0100, 0x0156
...
class orno:
    def __init__(self, port, slave_id=1, useMQTT=False, debug=False, log=True, logFile="", type=0):
        ...
        self.type = type
        self.defaultlogFile = datetime.now().strftime(
            f"{os.path.basename(__file__)}-%Y%m%d%H%M%S.log")
        self.port = port
        ...
        if self.log:
            try:
                if self.logFile == "":
                    ...
                    self.logFile = datetime.now().strftime(
                        f"{os.path.splitext(self.logFile)[0]}-%Y%m%d%H%M%S{os.path.splitext(self.logFile)[1]}")
    ...
    def logMessage(self, message):
        if self.log:
            try:
                self.txt = datetime.now().strftime(
    ...
    def query(self, register=0, decimals=2):
        if register == 0 and self.type == SDM72DV2:
            self.L1_voltage = self.read_float(
                L1_Voltage[self.type], 2, self.fc)
            self.L1_APower = self.read_float(
                L1_ActivePower[self.type], 2, self.fc)
            self.L2_voltage = self.read_float(
                L2_Voltage[self.type], 2, self.fc)
            self.L2_APower = self.read_float(
                L2_ActivePower[self.type], 2, self.fc)
            self.L3_voltage = self.read_float(
                L3_Voltage[self.type], 2, self.fc)
            self.L3_APower = self.read_float(
                L3_ActivePower[self.type], 2, self.fc)
            self.GridFrequency = self.read_float(
                GridFrequency[self.type], 2, self.fc)
            self.Net_Power = self.read_float(Net_Power[self.type], 2, self.fc)
            self.Total_Import_Active_Power = self.read_float(
                Total_Import_Active_Power[self.type], 2, self.fc)
            self.Total_Export_Active_Power = self.read_float(
                Total_Export_Active_Power[self.type], 2, self.fc)
            self.Total_Export_Power = self.read_float(
                Total_Export_Power[self.type], 2, self.fc)
        if register == 0 and self.type == WE514:
            self.L1_frequency = self.smartmeter.read_register(
                L1_Frequency[self.type], 2, self.fc)
            self.L1_voltage = self.smartmeter.read_register(
                L1_Voltage[self.type], 2, self.fc)
            self.L1_current = self.smartmeter.read_register(
                L1_Current[self.type], 3, self.fc)
            self.L1_power = self.L1_voltage * self.L1_current
            self.L1_PF = self.smartmeter.read_register(
                L1_PF[self.type], 3, self.fc)
            self.TotalPower = self.smartmeter.read_register(
                TotalPower[self.type], 2, self.fc)
            self.L1_APower = self.smartmeter.read_register(
                L1_ActivePower[self.type], 3, self.fc)
            self.L1_RPower = self.smartmeter.read_register(
                L1_ReactivePower[self.type], 3, self.fc)
            self.L1_ApPower = self.smartmeter.read_register(
                L1_ApparentPower[self.type], 3, self.fc)
        elif register == 0 and self.type == WE517:
            self.L1_frequency = self.read_float(
                L1_Frequency[self.type], 2, self.fc)
            self.L1_voltage = self.read_float(
                L1_Voltage[self.type], 2, self.fc)
            self.L1_current = self.read_float(
                L1_Current[self.type], 2, self.fc)
            self.L1_power = self.L1_voltage * self.L1_current
            self.L1_PF = self.read_float(L1_PF[self.type], 2, self.fc)
            self.L1_APower = self.read_float(
                L1_ActivePower[self.type], 2, self.fc)
            self.L1_RPower = self.read_float(
                L1_ReactivePower[self.type], 2, self.fc)
            self.L1_ApPower = self.read_float(
                L1_ApparentPower[self.type], 2, self.fc)
            self.L1_AEnergy = self.read_float(
                L1_ActiveEnergy[self.type], 2, self.fc)
            self.L1_FAEnergy = self.read_float(
                L1_ForwardActiveEnergy[self.type], 2, self.fc)
            self.L1_RAEnergy = self.read_float(
                L1_ReverseActiveEnergy[self.type], 2, self.fc)
            self.L2_frequency = self.read_float(
                L1_Frequency[self.type], 2, self.fc)
            self.L2_voltage = self.read_float(
                L2_Voltage[self.type], 2, self.fc)
            self.L2_current = self.read_float(
                L2_Current[self.type], 2, self.fc)
            self.L2_power = self.L2_voltage * self.L2_current
            self.L2_PF = self.read_float(L2_PF[self.type], 2, self.fc)
            self.L2_APower = self.read_float(
                L2_ActivePower[self.type], 2, self.fc)
            self.L2_RPower = self.read_float(
                L2_ReactivePower[self.type], 2, self.fc)
            self.L2_ApPower = self.read_float(
                L2_ApparentPower[self.type], 2, self.fc)
            self.L2_AEnergy = self.read_float(
                L2_ActiveEnergy[self.type], 2, self.fc)
            self.L2_FAEnergy = self.read_float(
                L2_ForwardActiveEnergy[self.type], 2, self.fc)
            self.L2_RAEnergy = self.read_float(
                L2_ReverseActiveEnergy[self.type], 2, self.fc)
            self.L3_frequency = self.read_float(
                L3_Frequency[self.type], 2, self.fc)
            self.L3_voltage = self.read_float(
                L3_Voltage[self.type], 2, self.fc)
            self.L3_current = self.read_float(
                L3_Current[self.type], 2, self.fc)
            self.L3_power = self.L3_voltage * self.L3_current
            self.L3_PF = self.read_float(L3_PF[self.type], 2, self.fc)
            self.L3_APower = self.read_float(
                L3_ActivePower[self.type], 2, self.fc)
            self.L3_RPower = self.read_float(
                L3_ReactivePower[self.type], 2, self.fc)
            self.L3_ApPower = self.read_float(
                L3_ApparentPower[self.type], 2, self.fc)
            self.L3_AEnergy = self.read_float(
                L3_ActiveEnergy[self.type], 2, self.fc)
            self.L3_FAEnergy = self.read_float(
                L3_ForwardActiveEnergy[self.type], 2, self.fc)
            self.L3_RAEnergy = self.read_float(
                L3_ReverseActiveEnergy[self.type], 2, self.fc)
            self.TotalPower = self.read_float(
                Total_ActiveEnergy[self.type], 2, self.fc)
            self.TotalActivePower = self.read_float(
                Total_ActivePower[self.type], 2, self.fc)
            self.TotalReactivePower = self.read_float(
                Total_ReactivePower[self.type], 2, self.fc)
            self.TotalApparentPower = self.read_float(
                Total_ApparentPower[self.type], 2, self.fc)
            self.TotalPF = self.read_float(Total_PF[self.type], 2, self.fc)
            self.TotalActiveEnergy = self.read_float(
                Total_ActiveEnergy[self.type], 2, self.fc)
            self.TotalForwardActiveEnergy = self.read_float(
                Total_ForwardActiveEnergy[self.type], 2, self.fc)
            self.TotalReverseActiveEnergy = self.read_float(
                Total_ReverseActiveEnergy[self.type], 2, self.fc)
            self.T1_TotalActiveEnergy = self.read_float(
                T1_TotalActiveEnergy[self.type], 2, self.fc)
            self.T1_ForwardActiveEnergy = self.read_float(
                T1_ForwardActiveEnergy[self.type], 2, self.fc)
            self.T1_ReverseActiveEnergy = self.read_float(
                T1_ReverseActiveEnergy[self.type], 2, self.fc)
            self.T2_TotalActiveEnergy = self.read_float(
                T2_TotalActiveEnergy[self.type], 2, self.fc)
            self.T2_ForwardActiveEnergy = self.read_float(
                T2_ForwardActiveEnergy[self.type], 2, self.fc)
            self.T2_ReverseActiveEnergy = self.read_float(
                T2_ReverseActiveEnergy[self.type], 2, self.fc)
            self.T3_TotalActiveEnergy = self.read_float(
                T3_TotalActiveEnergy[self.type], 2, self.fc)
            self.T3_ForwardActiveEnergy = self.read_float(
                T3_ForwardActiveEnergy[self.type], 2, self.fc)
            self.T3_ReverseActiveEnergy = self.read_float(
                T3_ReverseActiveEnergy[self.type], 2, self.fc)
            self.T4_TotalActiveEnergy = self.read_float(
                T4_TotalActiveEnergy[self.type], 2, self.fc)
            self.T4_ForwardActiveEnergy = self.read_float(
                T4_ForwardActiveEnergy[self.type], 2, self.fc)
            self.T4_ReverseActiveEnergy = self.read_float(
                T4_ReverseActiveEnergy[self.type], 2, self.fc)
    ...
    def print(self):
        if self.type == SDM72DV2:
            ...
            print(
                f"Total Import Power (Active) {self.Total_Import_Active_Power:.3f} W")
            print(
                f"Total Export Power (Active) {self.Total_Export_Active_Power:.3f} W")
            print(
                f"Total Export Power (Sum)    {self.Total_Export_Power:.3f} W")
        if self.type == WE514:
            print(f"L1 Voltage                  {self.L1_voltage:.0f} V")
            ...
            print(
                f"Total Active Power          {self.TotalActivePower:.2f} kWh")
            print(
                f"Total Reactive Power        {self.TotalReactivePower:.2f} kvar")
            print(
                f"Total Apparent Power        {self.TotalApparentPower:.2f} kva")
            print(f"Total PF                    {self.TotalPF:.2f} ")
            print(
                f"Total Active Energy         {self.TotalActiveEnergy:.2f} kWh")
            print(
                f"Total Forward Active Energy {self.TotalForwardActiveEnergy:.2f} kWh")
            print(
                f"Total Reverse Active Energy {self.TotalReverseActiveEnergy:.2f} kWh")
            print(
                f"T1 Total Active Energy      {self.T1_TotalActiveEnergy:.2f} kWh")
            print(
                f"T1 Forward Active Energy    {self.T1_ForwardActiveEnergy:.2f} kWh")
            print(
                f"T1 Reverse Active Energy    {self.T1_ReverseActiveEnergy:.2f} kWh")
            print(
                f"T2 Total Active Energy      {self.T2_TotalActiveEnergy:.2f} kWh")
            print(
                f"T2 Forward Active Energy    {self.T2_ForwardActiveEnergy:.2f} kWh")
            print(
                f"T2 Reverse Active Energy    {self.T2_ReverseActiveEnergy:.2f} kWh")
            print(
                f"T3 Total Active Energy      {self.T3_TotalActiveEnergy:.2f} kWh")
            print(
                f"T3 Forward Active Energy    {self.T3_ForwardActiveEnergy:.2f} kWh")
            print(
                f"T3 Reverse Active Energy    {self.T3_ReverseActiveEnergy:.2f} kWh")
            print(
                f"T4 Total Active Energy      {self.T4_TotalActiveEnergy:.2f} kWh")
            print(
                f"T4 Forward Active Energy    {self.T4_ForwardActiveEnergy:.2f} kWh")
            print(
                f"T4 Reverse Active Energy    {self.T4_ReverseActiveEnergy:.2f} kWh")

    def doLoop(self, count=0, infinite=True):
        if self.useMQTT and not self.isMQTT_connected:
            self.mqtt_enable()
            self.logMessage(
                f"Enabling MQTT connection: {self.isMQTT_connected}")
        if self.debug:
            self.logMessage(
                f"Start polling every {self.polling_interval} seconds slave id '{self.slave_id}'")
    ...
    def mqtt_prepareTopics(self, type=0):
        if type == SDM72DV2:
            ...
            self.TEP = f"{self.mqtt_topic}/TotalExportPower"
    ...
    def mqtt_enable(self):
        ...
        if self.debug:
            self.logMessage(
                f"Using MQTT Broker: '{self.mqtt_broker}:{self.mqtt_port}' with user '{self.mqtt_username}' and secret '{self.mqtt_password}'")
        ...
        try:
            if self.client.connected_flag:
                if self.type == SDM72DV2:
                    ...
                    self.client.publish(
                        self.IP,  f"{self.Total_Import_Active_Power}")
                    self.client.publish(
                        self.EP,  f"{self.Total_Export_Active_Power}")
                    self.client.publish(self.TEP, f"{self.Total_Export_Power}")
                if self.type == WE514:
                    self.client.publish(self.L1U, f"{self.L1_voltage}")
                ...
                if self.type == WE517:
                    ...
                    self.client.publish(
                        self.TApP, f"{self.TotalApparentPower}")
                    self.client.publish(self.TPF, f"{self.TotalPF}")
                    self.client.publish(self.TAE, f"{self.TotalActiveEnergy}")
                    self.client.publish(
                        self.TFAE, f"{self.TotalForwardActiveEnergy}")
                    self.client.publish(
                        self.TRAE, f"{self.TotalReverseActiveEnergy}")
                    self.client.publish(
                        self.T1TAE, f"{self.T1_TotalActiveEnergy}")
                    self.client.publish(
                        self.T1TFAE, f"{self.T1_ForwardActiveEnergy}")
                    self.client.publish(
                        self.T1TRAE, f"{self.T1_ReverseActiveEnergy}")
                    self.client.publish(
                        self.T2TAE, f"{self.T2_TotalActiveEnergy}")
                    self.client.publish(
                        self.T2TFAE, f"{self.T2_ForwardActiveEnergy}")
                    self.client.publish(
                        self.T2TRAE, f"{self.T2_ReverseActiveEnergy}")
                    self.client.publish(
                        self.T3TAE, f"{self.T3_TotalActiveEnergy}")
                    self.client.publish(
                        self.T3TFAE, f"{self.T3_ForwardActiveEnergy}")
                    self.client.publish(
                        self.T3TRAE, f"{self.T3_ReverseActiveEnergy}")
                    self.client.publish(
                        self.T4TAE, f"{self.T4_TotalActiveEnergy}")
                    self.client.publish(
                        self.T4TFAE, f"{self.T4_ForwardActiveEnergy}")
                    self.client.publish(
                        self.T4TRAE, f"{self.T4_ReverseActiveEnergy}")
            else:
                ...
                if self.mqtt_actual_connection_try < self.mqtt_connect_retry_count:
                    ...
                    t.sleep(30)
                    self.logMessage(
                        f"mqtt_publish(): Error - retry {self.mqtt_actual_connection_try}")
                    self.mqtt_publish()
                else:
                    self.logMessage(
                        f"mqtt_publish(): Error - No more retries!")
        except Exception as err:
    ...
    def mqtt_on_disconnect(self, client, userdata, flags, rc=0):
        self.logMessage(
            f"mqtt_onDisconnect: DisConnected {flags}"+" result code: "+str(rc)+" {client_id}  ")
        if rc == 0:
    ...
    def mqtt_on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            self.logMessage(
                "ORNO/MQTT: Failed to connect, return code %d\n", rc)
